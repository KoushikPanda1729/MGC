...
interface UseActionReturn extends UseActionState {
  ...
  fetchFaqs: (opts?: { signal?: AbortSignal }) => Promise<FaqItem[] | null>;
  ...
  const fetchPolicy = React.useCallback(
    async (type: PolicyType, opts?: { signal?: AbortSignal }) => {
      ...
      try {
        ...
        console.warn("API fetch failed, using mock policy data:", err?.message);
        const mockPolicy = MOCK_POLICIES[type as keyof typeof MOCK_POLICIES];
  ...
  const fetchFaqs = React.useCallback(
    async (opts?: { signal?: AbortSignal }) => {
      ...
      try {
        ...
        const parsed = ApiEnvelopeSchema(z.array(FaqItemSchema)).safeParse(
          envelope
        );
